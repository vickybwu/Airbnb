...
#host_response_rate, price, cleaning fee to numerics 
...
nyc_districts <- geojson_read(fil, what="sp")
     ...
     #  nyc_districts has 2 variables, BoroCD is the district #
nyc_districts_map <- fortify(nyc_districts, region="BoroCD")
     

gg<- ggplot()+
     geom_point(data=Private_rooms, aes(x=longitude, y=latitude, colour="Private Room"),size=1)+
     geom_point(data=Entire_rooms, aes(x=longitude, y=latitude, colour="Entire Home"),size=3)+
     geom_point(data=Shared_rooms, aes(x=longitude, y=latitude, colour="Shared Room"),size=2)+
...
cols<-c("Private Room"="hotpink2", "Entire Home"="lightskyblue3", "Shared Room"="peachpuff2")


gg+scale_colour_manual(name="Room Type",values=cols)
